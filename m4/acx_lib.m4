      ...
      AS_VAR_SET_IF([acx_cv_found_lib[]Name], , [
        AS_IF([test m4_ifnblank(header_file, ["x${Name[]_CFLAGS}" = "x" -a]) "x${Name[]_LIBS}" = "x"], [
          ...
          m4_ifnblank([$5], [
            ...
            do
              AS_IF([test -d "${dir}/${acl_libdirstem}/pkgconfig" -a -z "`AS_ECHO $PKG_CONFIG_PATH | $GREP ${dir}/${acl_libdirstem}/pkgconfig`"], [
                PKG_CONFIG_PATH="${PKG_CONFIG_PATH}${acx_pkg_config_path_sep}${dir}/${acl_libdirstem}/pkgconfig"
            ...
            do
              AS_IF([test -d "${dir}" -a -z "`AS_ECHO $acx_search_lib_path_checked | $GREP :${dir}:`"], [
                ...
                m4_foreach_w([libnm], libfiles, [
                  define([libfnm],[patsubst(translit(libtitle,[./+-], [____]),[_*$])])
                  found_[]libfnm[]_lib=
dnl                  AC_MSG_CHECKING([if lib[]libtitle[] is $dir/${acl_libdirstem}/lib[]libnm[].$acl_shlibext])
                  if test -f "$dir/${acl_libdirstem}/lib[]libnm[].$acl_shlibext"; then
                    found_[]libfnm[]_lib="yes";
                  elif test -f "$dir/${acl_libdirstem}/lib[]libnm[].$acl_libext"; then
                    found_[]libfnm[]_lib="yes";
                  fi
                ])
                AS_IF(
                  [test m4_ifnblank(header_file, ["x${found_[]Name[]_include}" != "x" -a]) m4_apply([m4_join([ -a], m4_foreach_w([libnm], libfiles, [define([libfnm],[patsubst(translit(libtitle,[./+-], [____]),[_*$])]) "x${found_[]libfnm[]_lib}" != "x"]))])],
                  [
                    AC_MSG_RESULT([yes])
                    m4_ifnblank(header_file, [
                    ...
                    Name[]_LIBS="-L$dir/${acl_libdirstem} m4_normalize(m4_foreach_w([lib], libfiles, [ -l[]lib]))"
                    ACX_CHECK_LIB_COMPILE(libtitle, [], [$3], [$4])
                    AS_IF([test "x${found_[]Name[]}" = "xyes"], [break])
                    m4_ifnblank([$2], [
                        m4_foreach([libdep], [$2], [
                          Name[]_LIBS="-L$dir/${acl_libdirstem} m4_normalize(m4_foreach_w([lib], libfiles libdep, [ -l[]lib]))"
